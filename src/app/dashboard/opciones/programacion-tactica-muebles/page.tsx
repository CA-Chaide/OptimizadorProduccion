...
import type { Grupo, Linea, Restriccion } from '@/types/interfaces';
...
export const TacticalPlanMueblesSection: React.FC = () => {
    ...
    const [allLineas, setAllLineas] = useState<Linea[]>([]);
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            setIsLoading(true);
            try {
                const [gruposRes, restriccionesRes, lineasRes] = await Promise.all([
                    grupoService.getAll(),
    ...
    useEffect(() => {
        const fetchTiemposData = async () => {
            if (gruposMuebles.length === 0 || allLineas.length === 0) {
            ...
            const codigoGrupoMuebles = grupoMuebles.codigo_grupo;
            const lineasDelGrupo = allLineas.filter(l => l.codigo_grupo === codigoGrupoMuebles).map(l => l.nombre_linea.trim());

            if (lineasDelGrupo.length === 0) {
                addNotification('warning', `No se encontraron líneas para el grupo "${grupoMuebles.nombre_grupo}".`);
                setIsTiemposLoading(false);
                return;
            }

            try {
                // Fetch ALL data using pagination from the generic endpoint
                const BATCH_SIZE = 10000;
                let allTiempos: any[] = [];
                
                // Exploratory call to get total records
                const exploreRes = await serviciosService.getTiemposEnsamblado(1, 1);
                const totalRecords = exploreRes.totalRegistros || 0;

                if (totalRecords === 0) {
                     addNotification('warning', 'No se encontraron datos en TiemposEnsamblado.');
                     setIsTiemposLoading(false);
                     setTiemposMueblesData([]);
                     return;
                }
                ...
                for (let i = 1; i <= totalPagesToFetch; i++) {
                     addNotification('info', `Cargando lote de tiempos de ensamblado ${i} de ${totalPagesToFetch}...`);
                     const response = await serviciosService.getTiemposEnsamblado(i, BATCH_SIZE);
                ...
                const filteredData = allTiempos.filter(item => 
                    String(item.Centro).trim() === centro &&
                    lineasDelGrupo.includes(String(item.Linea).trim())
                );
                
                setTiemposMueblesData(filteredData);
                addNotification('success', `Tiempos de Muebles cargados: ${filteredData.length} registros relevantes encontrados.`);
        ...
        fetchTiemposData();
    }, [gruposMuebles, allLineas, addNotification, isLoading]);
