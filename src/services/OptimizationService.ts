...
export function processAndValidateAssemblyData(
    ...
    console.log("--- INICIANDO PROCESAMIENTO Y VALIDACIÓN DE DATOS DE ENSAMBLE ---", { apiDataCount: apiData.length });
...
function sequenceDailyProduction(
    ...
    currentDate: Date,
    dailyDemandTotals: Map<string, number> // Key: "productId---centerId", Value: Total demand for the entire month
): MonthlyAssignment[] {
    ...
    const scoredGoals = dailyGoals.map(goal => {
        ...
        // This calculates the average daily demand for the month, to avoid overreacting to daily spikes.
        const avgDailyDemand = (dailyDemandTotals.get(stockKey) || 1) / 30;

        // A simple urgency score: lower is more urgent.
        // It prioritizes items with low "days of stock" cover.
...
export const generateProductionPlan = async (
  ...
  for (let monthIndex = 0; monthIndex < planningHorizon.length; monthIndex++) {
    ...
    console.log(`--- Procesando Plan Diario para Mes ${month + 1}/${year}. ${assignmentsForMonth.length} asignaciones a procesar.`);
    
    const dailyDemandTotals = new Map<string, number>();
    demandMap.forEach((monthlyDemands, pairKey) => {
        const demand = monthlyDemands[`${year}-${month}`] || 0;
        dailyDemandTotals.set(pairKey, demand);
    });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        for (const [lineId, assignments] of assignmentsByLine.entries()) {
            ...
            for (const assignment of sequencedAssignments) {
                ...
                console.log(`    Línea ${line.name}: Produce ${Math.round(unitsToProduce)} u de ${productId}. Horas consumidas: ${hoursConsumed.toFixed(2)}. Horas restantes hoy: ${hoursRemainingToday.toFixed(2)}.`);
