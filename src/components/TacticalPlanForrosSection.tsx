...
export const TacticalPlanForrosSection: React.FC = () => {
    ...
    const [inventoryData, setInventoryData] = useState<InventoryForrosItem[]>([]);
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            // Filtros específicos para Forros (ejemplo de RespCtrlProd para forros)
            const [inventoryRes, skillsRes] = await Promise.all([
                queryApi({
                    source: 'CuboInventarios',
                    operation: 'get_data',
                    filters: { 'Sector': ['02 FORROS'] }, // Ajustar según catálogo real
            ...
            if (inventoryRes) {
                ...
                addNotification('info', `Se cargaron ${inventoryRes.length} registros de inventario para Forros.`);
            } else {
                addNotification('warning', 'No se encontraron datos de inventario para Forros.');
            }
            ...
            const errorMessage = `Error al consultar datos para Forros: ${err.message}`;
    ...
    return (
        <div className="p-6 md:p-8 space-y-6">
            <div className="flex items-center space-x-3">
                ...
                <h2 className="text-2xl font-semibold text-gray-700">Programación Táctica Diaria (Forros)</h2>
            </div>
            
            <Tabs defaultValue="inventory">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="inventory">Resumen de Inventario (Forros)</TabsTrigger>
                            ...
                            {qualifiedOperatorCount !== null && (
                                <div className="flex items-center p-2 bg-blue-50 border border-blue-200 rounded-lg">
                                    ...
                                    <span className="text-sm font-semibold text-blue-800">
                                        {qualifiedOperatorCount} operadores calificados
                                    </span>
                                </div>
                            )}
                        </div>
