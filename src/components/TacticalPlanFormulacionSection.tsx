...
export const TacticalPlanFormulacionSection: React.FC = () => {
  ...
  const curadoGroupsSummary = useMemo(() => {
    ...
    curadoRows.forEach(row => {
      ...
      const enriched = { ...row, apertura: info.apertura, densityFixed: densityVal, pesoNumeric: pesoVal };
      ...
      const isCofama = estadoTrasVal === 'BCALL' && maquinaVal === 'F_BLOQ_M' && pesoVal > 50;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-white min-h-screen rounded-xl border border-gray-100 shadow-sm font-sans text-left">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            disabled={isLoading} 
            size="sm"
            className="h-9 px-4 rounded-xl bg-primary hover:bg-primary/90 text-white gap-2 font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all"
          >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="h-9 px-4 rounded-xl border-gray-200 gap-2 font-black text-[10px] uppercase shadow-sm transition-all hover:border-primary/50">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-7 gap-y-1 text-center mb-4">
                      ...
                      {calendarDays.map((day, idx) => {
                        ...
                        return (
                          <button key={dStr} onClick={() => toggleDate(dStr)} className={cn("relative h-7 w-7 mx-auto rounded-xl flex items-center justify-center transition-all", isSel ? "bg-primary text-white shadow-md shadow-primary/20" : "hover:bg-slate-50")}>
                            <span className={cn("text-[10px] font-black", !datesWithOrders.has(dStr) && !isSel ? "text-slate-200" : "text-slate-700")}>{format(day, 'd')}</span>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="resumen" className="space-y-6 animate-in fade-in duration-300">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            {apertureReport.map((ap) => (
              <Card key={ap.apertura} className="p-3 border-none shadow-sm bg-slate-900 text-white rounded-2xl">
                <p className="text-[8px] font-black uppercase text-slate-500 tracking-wider">Apertura</p>
                <div className="flex justify-between items-end mt-1">
                  <p className="text-sm font-black font-mono tracking-tighter text-[#facc15]">{ap.apertura}</p>
                  <p className="text-sm font-black font-mono text-indigo-400">{ap.totalBloques}<span className="text-[8px] opacity-40 ml-0.5">BL</span></p>
                </div>
              </Card>
            ))}
          </div>

          <Card className="border border-gray-100 rounded-2xl shadow-xl overflow-hidden bg-white">
        ...
        <TabsContent value="curado" className="space-y-8 animate-in fade-in duration-300 text-left">
          {curadoGroupsSummary.map((group) => (
            <div key={group.id} className="space-y-3">
              <div className="flex justify-between items-end px-1">
                <div>
                  <h3 className="text-xs font-black text-gray-800 uppercase tracking-tighter flex items-center gap-2">
                ...
                <div className="text-right">
                  <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Total Segmento</p>
                  <p className="text-xs font-black text-slate-800">{group.rows.length} Bloques | {group.rows.reduce((s, r) => s + safeNum(r.peso || r.PESO || r.pesoNumeric), 0).toLocaleString()} KG</p>
                </div>
              </div>
                        ...
                        <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                          {Object.keys(row).map(k => (
                            <td key={k} className={cn(
                              "px-4 py-1.5 border-r border-gray-50",
        ...
        <TabsContent value="inventario" className="animate-in fade-in duration-300 space-y-4 text-left">
          <div className="flex items-center gap-2 px-1">
            <div className="p-1.5 bg-blue-600 rounded-lg text-white"><Database className="w-3.5 h-3.5" /></div>
