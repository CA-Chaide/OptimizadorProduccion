...
import { Scissors, Users, Lock, Package, Loader2, AlertCircle, Clock } from 'lucide-react';
...
export const TacticalPlanCorteLaminadoSection: React.FC = () => {
  ...
  // 1. Cargar Grupos de Corte y Laminado
  ...
  const fetchTiemposEnsamblado = async (filteredGroups: Grupo[]) => {
    try {
      ...
      inspector.captureVariable('tiemposEnsambladoLaminado', allTiempos);
    } catch (error) {
    ...
    <div className="p-6 md:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Gestión de procesos para el grupo "Corte y Laminado"</p>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="restricciones">
          <Card>
            <CardHeader>
              <CardTitle>Restricciones del Grupo</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <Badge variant="outline" className="bg-red-50 text-red-700">{ordenesFiltradas.length} Registros</Badge>
              </div>
            </CardHeader>
            <CardContent>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredTiempos.slice((tiemposCurrentPage - 1) * tiemposItemsPerPage, tiemposCurrentPage * tiemposItemsPerPage).map((t, idx) => (
                          <tr key={idx} className="hover:bg-blue-50/20 transition-colors">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-center text-red-600">{t.Tiempo_Min ? t.Tiempo_Min.toFixed(4) : 'N/A'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
