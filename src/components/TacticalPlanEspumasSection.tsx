...
import { Wind, Package, Loader2, Wrench, ShoppingCart, RefreshCw, MapPin, Clock, UserCheck } from 'lucide-react';
...
export const TacticalPlanEspumasSection: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      logger.log('[Corte Espuma] Sincronizando datos integrales y tiempos SAP...');
      ...
      logger.success(`[Corte Espuma] Sincronización completa. Base de datos actualizada.`);
    } catch (error) {
  ...
  // Filtrado y Segmentación de Órdenes Provisionales
  const provsFiltradasAlmacen = useMemo(() => {
    return ordenesProvisionales.filter(o => String(o.Almacen || o.ALMACEN || '').trim() === '1000');
  }, [ordenesProvisionales]);

  const provsUIO = useMemo(() => {
    return provsFiltradasAlmacen.filter(o => String(o.Centro || o.CENTRO || '').trim() === '1000');
  }, [provsFiltradasAlmacen]);

  const provsGYE = useMemo(() => {
    return provsFiltradasAlmacen.filter(o => String(o.Centro || o.CENTRO || '').trim() === '2000');
  }, [provsFiltradasAlmacen]);
  ...
  const getTiempoMaterial = (material: string, centro: string) => {
    ...
    return match ? safeNum(match.Tiempo || match.Tiempo_Min) : null;
  };

  const renderTable = (data: any[], title: string, badgeColor: string) => (
    <div className="space-y-4">
      <div className="flex items-center gap-2 px-2">
        <div className={cn("w-2 h-2 rounded-full", badgeColor)} />
          ...
          <table className="min-w-full divide-y divide-gray-200 text-[10px]">
            <thead className="bg-[#f8fafc] text-slate-500 uppercase font-black tracking-wider border-b border-gray-100 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-3 py-4 text-center">Resp. CP</th>
                <th className="px-3 py-4 text-right">Cantidad</th>
                <th className="px-3 py-4 text-right bg-blue-50/50 text-blue-700">Tiempo (Min)</th>
            ...
            <tbody className="divide-y divide-gray-50 font-bold text-slate-700">
              {data.length === 0 ? (
                <tr><td colSpan={9} className="py-16 text-center text-slate-200 uppercase tracking-widest italic">Sin registros para el almacén 1000</td></tr>
              ) : (
                data.map((o, idx) => {
                  const tiempo = getTiempoMaterial(o.MATERIAL || o.CodMaterial, String(o.Centro || o.CENTRO));
                  return (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-2 text-right font-mono text-blue-600 bg-blue-50/20">
                        {tiempo ? tiempo.toFixed(2) : <span className="text-slate-300">N/D</span>}
                      </td>
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-100">
        <div className="flex items-center space-x-3 text-left">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Gestión Centralizada - Auditoría Almacén 1000</p>
          </div>
        </div>
        <Button 
          onClick={fetchData} 
        ...
        >
          {isLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />} SINCRO SAP
        </Button>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-3 h-10 bg-gray-50/80 p-1 rounded-xl border border-gray-100 mb-8">
          <TabsTrigger value="provisionales" className="gap-2 text-[9px] font-black uppercase transition-all data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-primary rounded-lg">
            <Package className="w-3.5 h-3.5" /> PROVISIONALES (ALM 1000)
          </TabsTrigger>
          ...
          {isLoading ? (
            ...
            <>
              <TabsContent value="provisionales" className="animate-in fade-in duration-300 space-y-10">
                {renderTable(provsUIO, "CORTE ESPUMA UIO (Planta 1000)", "bg-green-600")}
                {renderTable(provsGYE, "CORTE ESPUMA GYE (Planta 2000)", "bg-indigo-600")}
              </TabsContent>
