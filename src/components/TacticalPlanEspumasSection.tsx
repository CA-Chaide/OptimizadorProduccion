    ...
    return data.map((o, i) => {
      ...
      return (
        <tr key={i} className="hover:bg-gray-50/50 transition-colors text-center text-[10px]">
          ...
          <td className="px-3 py-2 font-mono font-bold border-r border-gray-100 text-teal-600 bg-teal-50/10">
    ...
    <div className="p-4 md:p-6 space-y-6 bg-white min-h-screen rounded-xl border border-gray-100 shadow-sm font-sans">
      <div className="flex items-center justify-between pb-4 border-b border-gray-100">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium">Gestión de Cargas Carrusel</p>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="resumen" className="space-y-8 animate-in fade-in duration-300">
                ...
                <div className="bg-white p-3">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-1 bg-gray-50 rounded-lg p-1">
                      <Button variant="ghost" size="icon" onClick={() => setViewDate(subMonths(viewDate, 1))} className="h-6 h-6 hover:bg-white hover:shadow-sm">
                        <ChevronLeft className="w-3 h-3" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => setViewDate(addMonths(viewDate, 1))} className="h-6 h-6 hover:bg-white hover:shadow-sm">
                  ...
                  <div className="grid grid-cols-7 gap-y-1 text-center mb-2">
                    {['LU', 'MA', 'MI', 'JU', 'VI', 'SA', 'DO'].map((day, idx) => (
                      <div key={`cal-head-${idx}`} className="text-[8px] font-bold text-gray-300 uppercase py-1">{day}</div>
                    ))}
                    {calendarDays.map((day, idx) => {
                      ...
                      return (
                        ...
                        >
                          <span className={cn("text-[10px] font-bold", !hasData && !isSelected ? "text-gray-200" : "")}>
                            {format(day, 'd')}
                          </span>
          ...
          {[ 
            { t: 'Planta 1000 - Quito (Almacén 1006)', d: summaryData1000, s: scrollResumen1000, c: 'text-green-700', b: 'bg-green-600', totals: summaryTotals1000 }, 
                ...
                <div ref={center.s.bottom} className="overflow-x-auto max-h-[400px]">
                  <table ref={center.s.table} className="w-full border-collapse text-center">
                    ...
                    <tbody className="divide-y divide-gray-50 text-[11px]">
                      {center.d.length === 0 ? (
                        <tr><td colSpan={8} className="py-10 text-center text-gray-300 italic">Sin demanda para el período</td></tr>
                      ) : (
                        center.d.map((row, i) => (
                  ...
                  <table ref={center.s.table} className="w-full border-collapse text-center">
                    <thead className="bg-gray-100/80 sticky top-0 z-10 text-[9px] font-bold uppercase text-gray-500 border-b border-gray-100">
                      <tr>
                        ...
                        <th className="px-2 py-4 border-r border-gray-100 text-gray-800">LARGO</th>
                        <th className="px-2 py-4 border-r border-gray-100 text-gray-800">ESP.</th>
                        ...
                        <th className="px-2 py-4 border-r border-gray-100 bg-orange-50/10">SUBBL.</th>
                        <th className="px-2 py-4 border-r border-gray-100 bg-orange-50/10 font-bold">BLOQUES</th>
                        <th className="px-2 py-4 border-r border-gray-100 bg-purple-50/10 font-bold">CARGAS</th>
              ...
              <div key={center.id} className="space-y-4">
                ...
                <Card className="rounded-2xl border border-gray-100 shadow-sm overflow-hidden bg-white">
                  <table className="w-full border-collapse text-center">
                    ...
                    <tbody className="divide-y divide-gray-50 text-[11px]">
                      {list.length === 0 ? (
                        <tr><td colSpan={3} className="py-10 text-center text-gray-300 italic">Sin restricciones configuradas</td></tr>
                      ) : (
                        list.map(r => (
                          <tr key={r.codigo_restriccion} className="hover:bg-gray-50/50 transition-colors">
                            <td className="px-6 py-3 font-bold text-gray-700 border-r border-gray-50 uppercase">{r.nombre_restriccion}</td>
                            ...
                            <td className="px-6 py-3 text-gray-500 italic text-left">{r.descripcion || '—'}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
        ...
        <TabsContent value="tiempos" className="space-y-10">
          {[ 
            { t: 'Catálogo Quito 1000', d: tiemposC1000, s: scrollTiempos1000, c: 'text-teal-700', b: 'bg-teal-600' }, 
                  ...
                  <table ref={center.s.table} className="w-full border-collapse text-center">
                    <thead className="bg-gray-100/50 sticky top-0 z-10 text-[9px] font-bold uppercase text-gray-400 border-b border-gray-100">
                      <tr>
                        ...
                        <th className="px-6 py-4 border-r border-gray-50 text-left">Descripción</th>
                        ...
                        <th className="px-6 py-4">Stock / Seguridad</th>
                      </tr>
                    </thead>
                      ...
                      {center.d.map((t, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                            <td className="px-6 py-3 font-mono font-bold text-primary border-r border-gray-50">{info.code}</td>
                            <td className="px-6 py-3 text-left border-r border-gray-50 text-gray-500 uppercase truncate max-w-[280px]">{info.desc}</td>
                            ...
                            <td className="px-6 py-3 text-gray-400 font-mono">{(t.StockActual || 0).toLocaleString()} / {(t.StockSeguridad || 0).toLocaleString()}</td>
                          </tr>
                        );
                      })}
                    </tbody>
