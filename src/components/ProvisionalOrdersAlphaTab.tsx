...
import { ClipboardList, Loader2, Search, Package, LayoutDashboard } from 'lucide-react';
...
export const ProvisionalOrdersAlphaTab: React.FC<ProvisionalOrdersAlphaTabProps> = ({ restricciones = [] }) => {
    ...
    const fetchData = async (page: number, rows: number) => {
        ...
        try {
            ...
            addNotification('error', 'Error al cargar las órdenes del plan táctico');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="space-y-4">
            ...
            {isLoading && data.length === 0 ? (
                <div className="flex justify-center items-center py-20">
                    ...
                    <span className="ml-3 text-gray-600 font-medium">Cargando Plan Táctico...</span>
                </div>
            ) : data.length > 0 ? (
                <>
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-gray-50 border-2 border-dashed rounded-xl">
                    ...
                    <p className="text-gray-500">No se encontraron datos para el Plan Táctico.</p>
                </div>
            )}
        </div>
    );
