...
export const MaestroMaterialesExplosionSection: React.FC = () => {
  ...
  const fetchData = useCallback(async (page: number, limit: number, search: string) => {
    ...
    setError(null);
    try {
      logger.log(`[MaestroMateriales] Consultando Explosión BOM para material: ${search}...`);
      ...
      setError(msg);
      addNotification('error', `Error al cargar explosión de materiales: ${msg}`);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Consulta Jerárquica de Componentes</p>
          </div>
        </div>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="py-32 text-center text-gray-400 space-y-4">
                      ...
                      <p className="font-bold uppercase tracking-widest text-[10px]">No se encontró el material "{activeSearch}"</p>
                    </td>
                  </tr>
                ) : (
                  data.map((item, idx) => (
      ...
      {hasStarted && !isLoading && !error && totalPages > 1 && (
        <div className="flex items-center justify-between px-2 pt-4 pb-6">
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
            Mostrando {data.length} de {totalRecords.toLocaleString()} componentes
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 mx-4 px-4 py-1 bg-gray-100 rounded-xl">
